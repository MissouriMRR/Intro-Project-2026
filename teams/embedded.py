...
# Vendored helper modules (path_finder.py, bfs_solver.py) live in
# teams/_embedded_lib/ so the scorer's teams/*.py glob does not pick them up as
# solvers. Put that dir on sys.path so they import when we're loaded by path.
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "_embedded_lib"))
